...
from ui.Ui_empleado_tipo import Ui_MainWindow
...
class EmpleadoTipo(QMainWindow, Ui_MainWindow):
	"""
	...
	def __init__(self, parent = None):
		...
		self.search.tabla='cf_empleado_tipo'		
	...
	def fill_data(self, id=None):				
		...
		if self.datos.check_havedata('cf_empleado_tipo')==True	:
			...
			self.emp = self.datos.get_data('cf_empleado_tipo', id)		
	...
	def on_actionGuardar_triggered(self):
		...
		if self.isadding:
			...
			self.lastid=self.datos.save_empleado_tipo(datos)
			self.mensaje='Datos salvados con exito'			
		elif self.isediting:
			datos=[self.lineEdit.text(), self.empid]
			self.datos.update_empleado_tipo(datos)
